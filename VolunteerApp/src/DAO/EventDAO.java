...
public class EventDAO {
	...
	public int addEventDets(String name, int createdBy, String date, String startTime, String endTime, String img,
		...
		String query = " INSERT INTO Event_Det (Name, Created_By, Event_Date, Start_Time, End_Time, img, Details) Output Inserted.Event_Det_ID "
				+ "Values('" + name + "', " + createdBy + " , '" + date + "', '" + startTime + "', '" + endTime + "', '"
				+ img + "', '" + description + "' )";
	...
	public void setEventLocations(int primaryKey, String county, String location, int spaces,
			ArrayList<List<String>> aList) throws Exception {

		StringBuilder sb = new StringBuilder(
				"  Insert Into Event_Loc(Location, County, Event_Det_ID, Location_Manager, Available_Spaces)\r\n"
	...
	public Vector<Event> getAvailability(int Event_id, List<String> timeList, List<String> endList) throws Exception {
		...
		for (int i = 0; i < timeList.size(); i++) {
			...
			// String query = "Select Count(e.Users_ID) as Volunteers, l.Available_Spaces
			// from Event e inner join Event_Loc l on e.Event_Loc_ID = l.Event_Loc_ID where
			// e.Event_Loc_Id = "+Event_id+" and '"+timeList.get(i)+"' between
			// e.Available_Start and e.Available_End group by l.Available_Spaces";

			String query = "Select (Select Count(Users_ID) from Event where event_Loc_ID = " + Event_id
					+ " and Available_Start <= '" + timeList.get(i) + "' and '" + timeList.get(i)
					+ "' < Available_End), Available_Spaces From Event_Loc where event_Loc_ID = " + Event_id + " ";
	...
	public Event getUpdateEventInfo(int id) throws Exception {
		...
		String query = "Select Name, Created_By, Event_Date, LEFT(Start_Time,5), LEFT(End_Time,5), img, Details, Event_Det_ID from Event_Det where Event_Det_Id = "
				+ id + " ";
	...
	public Vector<Event> getUpdateLocaitons(int id) throws Exception {
		...
		String query = "Select lower(County), Location, Available_Spaces, Event_Loc_ID from Event_Loc where Event_Det_Id = "
				+ id + " ";
	...
	public void updateEventInfo(String eventName, String eventDate, String startTime, String endTime, String img,
			String description, int id) throws Exception {

		DBManager dbmgr = new DBManager();
		Connection conn = dbmgr.getConnection();

		String query = "Update Event_Det set Name = '" + eventName + "', Event_Date = '" + eventDate
				+ "', Start_Time = '" + startTime + "',	End_Time = '" + endTime + "', img = '" + img + "' , Details = '"
				+ description + "' where Event_Det_ID = " + id + " ";
	...
	public void updateLoactionInfo(ArrayList<List<String>> upList) throws Exception {
		...
		for (int i = 0; i < upList.size(); i++) {
			for (int j = 0; j < 1; j++) {
				...
				sb.append("Update Event_Loc set Location = '" + loc + "' , County = '" + con + "' , Available_Spaces = "
						+ space + " Where Event_Loc_ID = " + id + " ");
			}
	...
	public void addNewUpdateEvent(int id, ArrayList<List<String>> aList) throws Exception {
		...
		StringBuilder sb = new StringBuilder(
				"Insert Into Event_Loc(Location, County, Event_Det_ID, Location_Manager, Available_Spaces) Values");
	...
	public static void updateUserVolunteerHours(int id, String start, String end) throws Exception {
		...
		String query = "Update event set Available_Start = '" + start + "', Available_End = '" + end
				+ "' where Event_ID = " + id + " ";
	...
	public Vector<Event> getLocAndNumber(int id) throws Exception {
		...
		String query = " Select l.Location,\r\n" + 
				"				ISNULL((Select Count(*) from Event where event_Loc_Id = l.Event_Loc_ID group by Event_LOC_ID),0)\r\n" + 
				"				from Event_Loc l\r\n" + 
				"				where l.Event_Det_ID  = "+ id +"  ";
	...
	public void updateLoactionAmount(ArrayList<List<String>> upList) throws Exception {
...
//				String con = upList.get(i).get(j + 1);
				...
				int id = Integer.parseInt(upList.get(i).get(j));
				double amt = Double.parseDouble(upList.get(i).get(j + 1));
				
				System.out.println(amt);

				sb.append("Update Event_Loc set Raised = " + amt + "  Where Event_Loc_ID = " + id + " ");
			}
	...
	public Vector<Event> getEventMoney(int id) throws Exception {
		...
		// https://stackoverflow.com/questions/28872787/how-to-get-current-todays-date-data-in-sql-server
		String query = "Select l.Location, l.Raised from Event_Loc l\r\n" + 
				"where Event_det_Id = "+ id +" ";
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Error in Event.getEventMoney ");
		}
		return eventData;
